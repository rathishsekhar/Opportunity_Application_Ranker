...
model_selectbox = st.sidebar.selectbox(
    "Select your model",
    ("TFIDF weighted Word2Vector", "BERT based (uncased)", "DistilBERT based (uncased)")
)

stack_selectbox = st.sidebar.selectbox(
    "Select how want to stack the encoded vectors", ("Horizontal", "Vertical")
)

# Setting model and stacking methods
model = 'dbert' if model_selectbox == "DistilBERT based (uncased)" else 'bert' \
      if model_selectbox == "BERT based (uncased)" else 'w2v'
stack = 'horizontal' if stack_selectbox == 'Horizontal' else 'vertical'
...
def main():
    ...
    if st.button("Confirm", key='Confirmation'):
        ...
        # Applying preprocessing steps - according to the model:
        if model in ('bert', 'dbert'):
            ...
            for colnames in [col for col in str_column if col in can_column]:
                df_data_dict[colnames+"__w2vpp"] = df_data_dict[colnames + "__pp"].apply(preprocessing_w2v)

            # Observed that preprocessing_w2v funtion creates 'None' (not NoneType)
            col__w2vpp = [col + "__w2vpp" for col in str_column if col in can_column]
            col__pp = [col + "__pp" for col in str_column if col in can_column]
            df_data_dict[col__w2vpp] = df_data_dict[col__w2vpp].replace({'None': ''})
        ...
        # Code to get bert based(i.e "BERT" or "distilBERT") models
        if model in ('bert', 'dbert'):
            ...
            can_model_dict_hstack_df, can_model_dict_vstack_df = modelbased_embedder(
                df_data_dict, uid_column_name, str_col, bool_col, float_col, hugging_face_model_name)
            data_vector = list(can_model_dict_hstack_df.values())[0] if stack == 'horizontal' else list(can_model_dict_vstack_df.values())[0]
            ...
            trans_df = pca_fit.transform(transformed_data_vector) if stack == 'horizontal' else transformed_data_vector
            trans_df = trans_df.reshape((-1,))
            
            # Getting the data containt job_applications for matching the values
            model_stack_dict = "opp__" + str(model) + '_pca_hstack_dict' if stack == 'horizontal' else "opp__" + str(model) + '_pca_vstack_dict'
            job_data = get_data(model_stack_dict)

        elif model == 'w2v':

            # Gathering w2v dictionaries obtrained from train data
            ApplicationId_w2v = get_data('ApplicationId_w2v_dict')
            ApplicationId_word2weight= get_data('ApplicationId_word2weight_dict')

            can_model_dict_hstack_df, can_model_dict_vstack_df = w2vbased_embedder(
                df_data_dict, uid_column_name, str_col, bool_col, float_col, ApplicationId_w2v, ApplicationId_word2weight
                )
            data_vector = list(can_model_dict_hstack_df.values())[0] if stack == 'horizontal' else list(can_model_dict_vstack_df.values())[0]
            transformed_data_vector = data_vector.reshape((1, -1))
            ...
            # Dimensionality reduction for hstack data
            # Gathering the pickle file which contains the pca.fit() for main data
            pca_fit_filename = "candidate_w2v_pca_model"
            pca_fit = get_data(pca_fit_filename)
        ...
        matched_job_dict = find_n_topmatches(trans_df, job_data, number_of_jobs)
